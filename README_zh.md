...
### 功能特性
- 多账号管理系统：动态个人资料卡片、底部工作表切换器支持滑动删除、过期账号指示器和自动登录触发。
- OAuth 登录支持多个账号；持久化令牌自动刷新；每个账号均支持长按进行凭据管理。
...
- 多账号管理：动态信息卡片；使用底部工作表切换器支持滑动删除；长按账号进行凭据管理；过期账号显示指示器并触发自动登录。
...
- 个人资料：重新设计带动态账号信息卡片；多账号切换器支持滑动删除；过期账号指示器；长按凭据管理；设置默认搜索视图，切换包含非排名 (s=any)，语言跟随系统设置，查看下载统计。
